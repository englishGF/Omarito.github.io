	...
	<div class="row">
		...
		<div class="col-sm-8">
			<?php 
				...
				<div>
					...
					<form action='profile.php?u_id=$user_id' method='post' enctype='multipart/form-data'>
						<ul class='nav pull-left' style='position:absolute;top:10px;left:40px;'>
							<li class='dropdown'>
								<button class='dropdown-toggle btn btn-defult' data-toggle='dropdown'>Change Cover</button>
								<div class='dropdown-menu'>
									<center>
									<p>Click <strong>Select Cover</strong> and then click the <br> <strong>Update Cover</strong></p>
									<label class='btn btn-info'>Select Cover<input type='file' name='u_cover' size='60'/></label><br><br>
				...
				<div id='profile-img'>
					<img src='users/$user_image' alt='Profile' class='img-circle' width='130px' height='135px'>
					<form action='profile.php?u_id=$user_id' method='post' enctype='multipart/form-data'>
					<label id='update_profile'>Select Profile<input type='file' name='u_image' size='60'/></label><br><br>
			...
			<?php 
				if (isset($_POST['submit'])) {
					...
					if ($u_cover=='') {
						echo "<script>alert('Please selct Cover Image')</script>";
		...
		<?php 
			if (isset($_POST['update'])) {
					...
					if ($u_image=='') {
						echo "<script>alert('Please selct Profile Image')</script>";
						echo "<script>window.open('profile.php?u_id=$user_id', '_self')</script>";
						exit();
					}else{
			...
			while ($row_posts = mysqli_fetch_array($run_posts)) {
				...
				if ($content == "No" && strlen($upload_image) >= 1) {
					...
					<div id='own_posts'>
						...
						<div class='row'>
							<div class='col-sm-12'>
								<iframe id='posts-img' src='imagepost/$upload_image' style='height:350px;'></iframe>
							</div>
						</div><br>
