...
$con = mysqli_connect("localhost","root","mysql","social_network") or die("Connection was not established");
...
function insertPost(){
	if(isset($_POST['sub'])){
		...
		$random_number = rand(1, 99999999999999);
			...
			if(strlen($upload_image) >= 1 && strlen($content) >= 1){
				...
				if($upload_image=='' && $content == ''){
					...
					if($content==''){
						...
						if($upload_image=='' && strlen($content) >= 1){
...
function single_post() {
	if (isset($_GET['post_id'])) {
		...
		$get_posts = "selct post_id from users where post_id='$post_id'";
